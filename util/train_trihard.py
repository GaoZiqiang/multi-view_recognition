...
from util.losses import CrossEntropyLoss, DeepSupervision, CrossEntropyLabelSmooth, TripletLossAlignedReID, TripletLoss
...
def main():
    ...
    # 优化器
    # param@model.parameters()：更新模型的所有参数　若更新某一层：model.conv1 model.fc；若更新两层：nn.Sequential([model.conv1,model.conv2])
...
def train(epoch,model,criterion_class,criterion_metric,optimizer,trainloader,use_gpu):
    ...
    # 训练的基本套路：
    # 1　导入数据；２　前向传播得到预测输出　３　计算前向传播所得预测输出与真实值的差值loss ４　反向传播，使用优化器进行梯度优化
    ...
    for batch_idx,(imgs,pids,_) in enumerate(trainloader):
        ...
        # 计算损失loss
        xent_loss = criterion_class(outputs,pids)#　对预测的outputs标签与实际标签pids进行计算差值，得到损失loss
