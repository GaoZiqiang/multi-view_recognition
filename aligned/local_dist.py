...
def shortest_dist(dist_mat):
  ...
  """
  m, n = dist_mat.size()[:2]# 取dist_mat.size()的前两个维度，m n
...
def batch_local_dist(x, y):
  """
  计算一个batch中所有images pair对的最小路径
  Args:
    x: pytorch Variable, with shape [N, m, d]N*8*128
    y: pytorch Variable, with shape [N, n, d]
  Returns:
    dist: pytorch Variable, with shape [N]
  """
  # 先对相关规格进行判断，防止下面的操作出错
  assert len(x.size()) == 3
  assert len(y.size()) == 3
  assert x.size(0) == y.size(0)
  assert x.size(-1) == y.size(-1)
  ...
  # shape [N, m, n]
  dist_mat = batch_euclidean_dist(x, y)# ???此处是要做递归吗？
  # 作归一化处理
...
if __name__ == '__main__':
    ...
    # 输入local feature，shape为：32*128*8*1
    # x,y为32个图片对
